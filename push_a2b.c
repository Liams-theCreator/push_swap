...
/*   Updated: 2025/02/13 19:17:05 by imellali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int find_position(t_lista *stack_b, int num)
{
	...
	if (num < smallest)
		return (find_idx(stack_b, smallest));
	head = stack_b;
	position = 0;
...
static void shift_and_push(t_lista **stack_a, t_lista **stack_b, int number)
{
	...
	if (index_a <= length_a / 2 && position_b <= length_b / 2)
	{
		while (index_a > 0 && position_b > 0)
		{
			rr(stack_a, stack_b);
			index_a--;
			position_b--;
	...
	else if (index_a > length_a / 2 && position_b > length_b / 2)
	{
